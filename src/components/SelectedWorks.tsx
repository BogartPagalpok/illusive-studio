...
export default function SelectedWorks() {
    ...
    const fetchData = async () => {
      ...
      if (data) {
        const sanitized: Project[] = data.map((item: any) => ({
          ...
          // DUAL-ASSET FALLBACK: If card_thumbnail is null in DB, use image_url
  ...
  return (
    ...
    >
      {/* 1. CINEMATIC BACKGROUND (Uses image_url) */}
          ...
          >
            ...
            <div className="relative flex-1 flex items-center justify-center overflow-hidden">
              <motion.div drag={zoom > 1} dragMomentum={false} dragElastic={0} className="flex items-center justify-center cursor-move">
                <motion.img animate={{ scale: zoom }} src={selectedProject.image_url} className="max-h-[85vh] w-auto object-contain pointer-events-none select-none" />
              </motion.div>
            </div>
          </motion.div>
        )}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .selected-works-container ~ nav { display: none !important; }
