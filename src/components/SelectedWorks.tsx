...
export default function SelectedWorks() {
  ...
  // BUILD FIX: Bulletproof Navbar Toggle
  useEffect(() => {
  ...
  return (
    <section id="works" className="relative h-screen w-full bg-[#0a0a0a] overflow-hidden flex flex-col justify-end">
      ...
      {/* 1. THE BILLBOARD (Synced to Project Thumbnail) */}
          ...
          {projects.length > 0 && projects[activeIndex] && (
            ...
            >
              <img 
                ...
                alt="billboard-bg"
              />
              {/* Theme-Corrected Left Side Contrast Tint */}
              <div className="absolute inset-0 bg-gradient-to-r from-[#0a0a0a] via-[#0a0a0a]/60 to-transparent" />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0a] via-transparent to-transparent" />
              <div className="absolute inset-y-0 left-0 w-1/3 bg-accent/10 blur-[160px] pointer-events-none" />
            </motion.div>
          )}
        </AnimatePresence>
      </div>

      {/* 2. LOWER-LEFT HERO CONTENT (Flush with Swiper) */}
        ...
        <AnimatePresence mode="wait">
          {projects.length > 0 && projects[activeIndex] && (
            ...
            >
              ...
              <h2 className="text-4xl md:text-6xl lg:text-[6.5vw] font-black text-white uppercase tracking-tighter leading-[0.8] italic mb-4">
          ...
          >
            <div className="flex justify-between items-center p-8 z-[10002]">
               ...
               <div className="flex gap-4">
                  <button onClick={() => setZoom(z => Math.max(z - 0.5, 1))} className="p-3 bg-white/5 rounded-full text-white border border-white/10 hover:bg-accent hover:text-black transition-all"><ZoomOut size={20}/></button>
                  <button onClick={() => setZoom(z => Math.min(z + 0.5, 4))} className="p-3 bg-white/5 rounded-full text-white border border-white/10 hover:bg-accent hover:text-black transition-all"><ZoomIn size={20}/></button>
            ...
            <div className="relative flex-1 flex items-center justify-center overflow-hidden">
               <button onClick={() => setCurrentImageIndex(prev => (prev - 1 + (selectedProject.all_images?.length || 1)) % (selectedProject.all_images?.length || 1))} className="absolute left-10 z-[10002] p-4 bg-black/60 rounded-full text-white hover:bg-accent transition-all"><ChevronLeft size={32}/></button>
               <button onClick={() => setCurrentImageIndex(prev => (prev + 1) % (selectedProject.all_images?.length || 1))} className="absolute right-10 z-[10002] p-4 bg-black/60 rounded-full text-white hover:bg-accent transition-all"><ChevronRight size={32}/></button>
