...
export default function SelectedWorks() {
  ...
  // Sync with Navbar's MutationObserver
  useEffect(() => {
  ...
  return (
    <section id="works" className="relative h-screen w-full bg-black overflow-hidden flex flex-col justify-end">
      {/* BACKGROUND BILLBOARD (image_url) */}
          ...
          <AnimatePresence mode="wait">
            {active && (
              <motion.div key={active.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                ...
                <button onClick={() => { setSelectedProject(active); setZoom(1); }} className="bg-white text-black px-10 py-4 rounded-md font-black uppercase text-xs hover:bg-accent transition-all flex items-center gap-2"><Play size={18} fill="black" /> VIEW PROJECT</button>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[99999] bg-black/40 backdrop-blur-[45px] flex flex-col">
            <div className="flex justify-between items-center p-8 z-[100] bg-black/10 backdrop-blur-md border-b border-white/5">
               ...
               <div className="flex gap-4">
                  <button onClick={() => setZoom(z => Math.max(z - 0.5, 1))} className="p-3 bg-white/5 rounded-full text-white border border-white/10 hover:bg-white/20"><ZoomOut size={20}/></button>
                  <button onClick={() => setZoom(z => Math.min(z + 0.5, 6))} className="p-3 bg-white/5 rounded-full text-white border border-white/10 hover:bg-white/20"><ZoomIn size={20}/></button>
            ...
            <div className="relative flex-1 flex items-center justify-center overflow-hidden">
               <button onClick={(e) => { e.stopPropagation(); const prevIdx = (projects.findIndex(p => p.id === selectedProject.id) - 1 + projects.length) % projects.length; setSelectedProject(projects[prevIdx]); setZoom(1); }} className="absolute left-8 z-[110] p-5 bg-white/5 backdrop-blur-md rounded-full text-white hover:bg-accent border border-white/10"><ChevronLeft size={40}/></button>
               <button onClick={(e) => { e.stopPropagation(); const nextIdx = (projects.findIndex(p => p.id === selectedProject.id) + 1) % projects.length; setSelectedProject(projects[nextIdx]); setZoom(1); }} className="absolute right-8 z-[110] p-5 bg-white/5 backdrop-blur-md rounded-full text-white hover:bg-accent border border-white/10"><ChevronRight size={40}/></button>
               <motion.div drag={zoom > 1} dragMomentum={false} dragElastic={0} className="flex items-center justify-center cursor-move">
                 <motion.img key={selectedProject.id} animate={{ scale: zoom }} src={selectedProject.image_url} className="max-h-[82vh] w-auto object-contain pointer-events-none select-none" />
               </motion.div>
            </div>
          </motion.div>
        )}
