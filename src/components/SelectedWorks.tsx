          ...
          >
            {projects.map((project) => (
              <SwiperSlide key={project.id} style={{ width: 'min(380px, 85vw)' }} className="!flex items-center justify-center">
                {({ isActive }) => (
                  <div className={`relative w-full rounded-[30px] border overflow-hidden backdrop-blur-3xl shadow-2xl transition-all duration-700 ease-out ${isActive ? 'h-[clamp(440px,60vh,620px)] border-white/20 bg-white/10 scale-100 z-10' : 'h-[clamp(380px,50vh,520px)] border-white/5 bg-white/5 scale-[0.85] opacity-60'}`}>
