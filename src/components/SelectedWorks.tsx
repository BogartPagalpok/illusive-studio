...
export default function SelectedWorks() {
  ...
  useEffect(() => {
    ...
    const uniqueProjects = filtered.filter((project, index, self) =>
      index === self.findIndex((p) => p.title.trim() === project.title.trim())
    );
              ...
              >
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  <button 
                    type="button"
                    onClick={() => setSelectedProject(currentProject)}
                    className="flex items-center gap-2 bg-white/20 text-white px-8 py-3 font-bold rounded backdrop-blur-md hover:bg-white/30 transition-all border border-white/20 shadow-lg"
                  >
      ...
      <AnimatePresence>
        {selectedProject && (
          ...
          >
            ...
            >
              <div className="border border-white/20 rounded-lg overflow-hidden flex flex-col gap-4 max-h-[80vh] overflow-y-auto no-scrollbar">
                {projects
                  .filter(p => p.title.trim() === selectedProject.title.trim())
                  .map(img => (
                    <img 
                      key={img.id} 
                      src={img.image_url} 
