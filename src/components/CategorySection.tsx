...
function getVideoEmbedUrl(url: string, platform: VideoPlatform): string {
  ...
  if (platform === 'tiktok') {
    return url; // TikTok embeds are complex, we'll open in new tab
  }
  return url;
}
        ...
        >
          ...
          >
            <div className="p-4">
              ...
              {project.description && (
                <p className="text-[var(--text-secondary)] text-xs mt-1 line-clamp-3">{project.description}</p>
              )}
              {project.tools && (
                  ...
                  <div className="mb-3">
                    ...
                    <div className="rounded-2xl overflow-hidden border" style={{ borderColor: 'var(--glass-border)', backgroundColor: 'var(--glass-bg)' }}>
                      {platform === 'tiktok' ? (
                        <PhoneFrame>
                          <div className="w-full h-full flex flex-col items-center justify-center bg-black/50 p-4">
                            ...
                            <a
                              ...
                              className="flex items-center gap-1 px-3 py-1.5 bg-accent text-white text-xs rounded-full font-bold hover:scale-105 transition-transform"
                              onClick={(e) => e.stopPropagation()}
                            >
                        ...
                        <MonitorFrame>
                          <iframe
                            src={getVideoEmbedUrl(project.video_url!, platform)}
                      ...
                      <div className="p-3">
                        ...
                        {project.description && (
                          <p className="text-[var(--text-secondary)] text-[10px] mt-1 line-clamp-2">{project.description}</p>
                        )}
                      </div>
                    </div>
                  </div>
