...
export default function DraggableCarousel({
  ...
  // Apply 3D transforms to every slide
  ...
  return (
    ...
    >
      ...
      {showArrows && (
        <>
          <button onClick={goPrev} aria-label="Previous slide" style={arrowButtonStyle(arrowSize, "left")}>
            <svg
          ...
          <button onClick={goNext} aria-label="Next slide" style={arrowButtonStyle(arrowSize, "right")}>
            <svg
              width={arrowSize * 0.45}
              height={arrowSize * 0.45}
              viewBox="0 0 24 24"
...
// ── Small style helpers ────────────────────────────────────
const arrowButtonStyle = (size: number, side: "left" | "right") =>
  ({
    position: "absolute",
    top: "50%",
