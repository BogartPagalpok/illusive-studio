            ...
            {count === 3 ? (
              ...
              <div className="grid grid-cols-3 gap-1 p-1">
                {displayImages.map((img, i) => (
                  <div
                    key={i}
                    className={`cursor-pointer overflow-hidden aspect-square relative ${i === 5 && remaining > 0 ? '' : ''}`}
...
// ── Original TitleGrid for Photography & UI/UX (unchanged) ──
  ...
  return (
    <>
      {visibleGroups.map(([title, titleProjects]) => {
        ...
        // ── Graphics: Composite Card ─────────────────────
        if (isGraphics && allImages.length > 0) {
          return (
